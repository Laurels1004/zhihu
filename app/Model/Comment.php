...
class Comment extends Model
{
    ...
    public function add(){
        ...
        if(rq('question_id')){
            ...
            if(!$question) return ['status'=>0, 'msg'=>'要评论的提问不存在!'];
            $this->question_id = rq('quesiton_id');
        } else {
            /*如果评论的是回答*/
            /*检查回答ID对应的回答是否存在*/
            $answer = answer_ins()->find(rq('answer_id'));
            if(!$answer) return ['status'=>0, 'msg'=>'要评论的回答不存在!'];
        ...
        if(rq('reply_to'))
        {
            ...
            if(!$target)
                return ['status'=>0, 'msg'=>'要回复的评论不存在!'];
//            if($target->user_id == session('uid'))
//                return ['status'=>0, 'msg'=>'用户不可自己评论自己!'];
    ...
    public function read(){
        ...
        if(rq('question_id')){
            /*对于查看问题的评论的处理*/
            $question = question_ins()->find(rq('question_id'));
            if(!$question) return ['status'=>0, 'msg'=>'查看评论对应的提问不存在'];
            $data = $this->where('question_id', rq('question_id'));
        } else {
            /*对于查看回答的评论的处理*/
            $answer = answer_ins()->find(rq('answer_id'));
            if(!$answer) return ['status'=>0, 'msg'=>'查看评论对应的回答不存在'];
    ...
    public function remove(){
        ...
        /*检查是否传递了评论的ID*/
        if(!rq('id'))
            return ['status'=>0, 'msg'=>'要删除的评论ID不能为空'];

        /*检查对应的评论是否存在*/
        $comment = $this->find(rq('id'));
        if(!$comment)
            return ['status'=>'0', 'msg'=>'要删除的评论不存在!'];

        /*检查当前用户是否是评论发起人*/
        if($comment->user_id != session('uid'))
            return ['status'=>0,'msg'=>'非评论发起者不能对该评论进行删除!'];
